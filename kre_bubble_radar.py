...
def run_brokerage_backtest(df, ticker='KRE', start_date='2009-01-01', dca_monthly=1000.0, cooldown_days=20):
    """
    ...
    for i in range(len(sub_bt)):
        ...
        if pos > 0 and (is_bub or is_bear) and (i - last_buy_idx >= cooldown_days):
            ...
            r_reason = '微观雷达泡沫与信贷过热' if is_bub else ('区域银行急性挤兑' if sub_bt.loc[i, 'Breadth_Regional_Banks'] <= 0.10 else '系统性信贷紧缩危机')
            trades.append({
                'action': 'SELL',
                'date': d_str,
                'price': p,
                ...
                'stress_spread': sub_bt['Bank_Stress_Spread'].iloc[i]
            })
            action_today = 'SELL'
            s_sh = 0.0
            pos = 0.0
        ...
        daily_records.append({
            ...
            'Bank_Stress_Spread': sub_bt['Bank_Stress_Spread'].iloc[i],
...
def export_deliverables(sub_bt, df_daily, df_pairs, metrics, ticker='KRE'):
    """
    导出机构级 Excel 审计全底稿与高清 4 层对齐图谱
    """
    excel_path = '宏观反身性阿尔法模型_KRE微观雷达全周期对账表.xlsx'
...
def main():
    ...
    sub_bt, df_daily, df_pairs, metrics = run_brokerage_backtest(radar.df, ticker='KRE', start_date='2009-01-01')
    ...
    export_deliverables(sub_bt, df_daily, df_pairs, metrics, ticker='KRE')
