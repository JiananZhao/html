...
def render_stock_tab():
    ...
    if ticker_to_analyze:
        with st.spinner(f"正在拉取 {ticker_to_analyze} 实时行情与基本面数据..."):
            from core_engine.stock_data_service import get_stock_price_data, get_stock_profile
            stock_df, price_meta = get_stock_price_data(ticker_to_analyze, period="5y")
            profile_res = get_stock_profile(ticker_to_analyze)
            stock_info = profile_res.get("data", {})

        has_price_data = stock_df is not None and not stock_df.empty
        has_info_data = stock_info is not None and bool(stock_info)

        if not has_price_data and not has_info_data:
            st.warning(f"未能获取到 {ticker_to_analyze} 的有效行情或基本面数据，上游接口受阻且本地无该标的离线序列。")
            with st.expander("🛠️ 查看诊断信息 (Diagnostics)", expanded=False):
                st.markdown(f"- **行情状态**: `{price_meta.get('status')}` - {price_meta.get('message')}")
                st.markdown(f"- **资料状态**: `{profile_res.get('status')}` - {profile_res.get('message')}")
            return

        is_local = price_meta.get("is_local_fallback", False)
        last_dt = price_meta.get("last_date", "")

        last_price = stock_df["Close"].iloc[-1] if has_price_data else np.nan

        if has_info_data and not is_local:
            curr_price = stock_info.get("currentPrice", stock_info.get("regularMarketPrice", last_price))
            price_title = "当前实时股价"
        elif is_local:
            curr_price = last_price
            price_title = f"历史收盘价 ({last_dt} · 本地离线)"
        else:
            curr_price = last_price
            price_title = "历史最后收盘价"

        comp_name = stock_info.get("shortName", ticker_to_analyze) if has_info_data else ticker_to_analyze
        sector = stock_info.get("sector", "N/A (未获取)") if has_info_data else "N/A"
        industry = stock_info.get("industry", "N/A (未获取)") if has_info_data else "N/A"
        pe_ttm = stock_info.get("trailingPE", np.nan) if has_info_data else np.nan
        pe_fwd = stock_info.get("forwardPE", np.nan) if has_info_data else np.nan
        ps_ttm = stock_info.get("priceToSalesTrailing12Months", np.nan) if has_info_data else np.nan
        pb_ratio = stock_info.get("priceToBook", np.nan) if has_info_data else np.nan
        market_cap = stock_info.get("marketCap", np.nan) if has_info_data else np.nan
        fcf = stock_info.get("freeCashflow", np.nan) if has_info_data else np.nan
        shares_out = stock_info.get("sharesOutstanding", np.nan) if has_info_data else np.nan
        eps_ttm = stock_info.get("trailingEps", np.nan) if has_info_data else np.nan
        beta = stock_info.get("beta", np.nan) if has_info_data else np.nan
        fcf_per_share = (fcf / shares_out) if (pd.notna(fcf) and pd.notna(shares_out) and shares_out > 0) else np.nan

        # 若资料缺失，客观提示用户，但不阻断技术面行情
        if not has_info_data:
            st.info(f"标的 `{ticker_to_analyze}` 基本面资料暂未获取到（{profile_res.get('message', '上游数据源无响应')}）。技术面行情正常展示。")

        # 若使用本地离线数据，明确标明
        if is_local:
            st.caption(f"⚠️ 当前显示为【{price_meta.get('price_type')}】，非实时行情。仅展示收盘价折线走势，不伪造K线。")

        st.subheader(f"🏢 {comp_name} ({ticker_to_analyze}) — {sector} | {industry}")

        # 核心估值卡片
        kpi1, kpi2, kpi3, kpi4, kpi5 = st.columns(5)
        kpi1.metric(price_title, f"${curr_price:.2f}" if pd.notna(curr_price) else "N/A")
        ...
        if has_price_data:
            ...
            # 严格遵照规范：若仅有收盘价 (如本地离线 fallback)，强制使用 Line 折线图，严禁伪造 K 线 (Requirement 5)
            if is_local or price_meta.get("is_close_only"):
                chart_type_arg = "Line"
                st.caption(f"💡 当前为本地历史数据，仅含收盘价序列，已锁定为折线图模式。")
            else:
